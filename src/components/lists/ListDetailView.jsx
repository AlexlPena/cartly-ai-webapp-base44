...
export default function ListDetailView({ listId, onBack, onDelete }) {
  ...
  const loadAll = async () => {
    setLoading(true);
    const [allLists, itemsData] = await Promise.all([
      base44.entities.List.filter({ id: listId }),
      base44.entities.ListItem.filter({ list_id: listId }, "category"),
    ]);
    const l = Array.isArray(allLists) ? allLists[0] : allLists;
    setList(l || null);
    setNameVal(l?.name || "");
    setItems(itemsData || []);
    setLoading(false);
  ...
  const addItem = async (data) => {
    const item = await base44.entities.ListItem.create({ ...data, list_id: listId, is_checked: false });
  ...
  const saveName = async () => {
    ...
    const updated = await base44.entities.List.update(listId, { name: nameVal.trim() });
    setList(updated);
  ...
  if (!list) {
    return <div className="text-center py-24 text-gray-400 text-sm">List not found.</div>;
  }
      ...
      <div className="flex items-center gap-3 mb-6">
        ...
        <div className="flex-1">
          ...
          <p className="text-sm text-gray-400 mt-0.5">
            {items.length} item{items.length !== 1 ? "s" : ""} · {progress}% done
          </p>
        </div>
        <button
